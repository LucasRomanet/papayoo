...
class Partie extends Component {
    ...
    componentDidMount() {
        this.socketFunction();
        let flickSize, hand = [...this.props.location.state.hand.hand];
        switch(this.props.location.state.player.length){
        ...
        this.setState({
            ...
            cards: {
                hand: this.playable(hand.sort(byId)),
                pool: [],
                original: this.props.location.state.hand.hand
            },
            highest : {
                id:-1,
                number:-1,
                        ...
                        this.state.player.map((p, index) =>
                            <div className={[(UserProfile.nametag(p) === this.state.mustPlay) ? "must-play":"", "player-wrapper"].join(' ')} >
                                <div className="opponent" index={index}>
                                    <a key={index} onClick={() => this.handleModalPlayerProfileOpen(index)} style={{cursor: 'pointer'}} >{p.name}#{p.tag}</a>
                                </div>
                                <div className="opponent" index={index}>
